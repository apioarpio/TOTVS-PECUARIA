...
import MACAdress from '../../config/MACAddress';
...
export default {
    ...
    create: function (codEstacao) {
        return new Promise(async (resolve, reject) => {
            const MAC_CURRENT_STATION = MACAdress.getMACAddress().replace(/:/g, '');
            const db = database();
            try {
                ...
                db.serialize(() => {
                    db.run(`INSERT INTO contexto VALUES(${codEstacao},null, '${MAC_CURRENT_STATION}')`,
                        (err, result) => {
                            if (err) {
                                console.log(err);
                                reject(err);
            ...
            if (codFazenda) {
                ...
                try {
                    ...
                    db.serialize(() => {
                        ...
                        WHERE cod_estacao = '${contexto.codEstacao}' and mac_address = '${contexto.macAddress}'
                    `, (err, result) => {
                            if (err) {
                                console.log(err)
    ...
    getContexto: function () {
        return new Promise((resolve, reject) => {
            const db = database();
            const macAddress = MACAdress.getMACAddress().replace(/:/g, '');
        ...
        return new Promise(async (resolve, reject) => {
            ...
            db.serialize(() => {
                ...
                db.all(`
                        ...
                        WHERE c.mac_address = '${contexto.macAddress}' ${query}
                        `, (err, result) => {
                    if (err) {
                        console.log(err);
