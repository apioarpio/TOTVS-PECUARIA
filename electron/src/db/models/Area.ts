...
export default {
    ...
    getAll(idFazenda, params) {
        return new Promise((resolve, reject) => {
            const db = database();
            db.serialize(() => {
                db.all(`SELECT * FROM area WHERE id_fazenda = ${idFazenda}`, (err, result) => {
                    if (err) {
                        console.log(err);
                        reject(err)
                    } else {
    ...
    getById(idArea, idFazenda) {
        return new Promise((resolve, reject) => {
            const db = database();
            db.serialize(() => {

                db.get(`SELECT * FROM area WHERE id_area = ${idArea} AND id_fazenda = ${idFazenda}`, (err, result) => {
                    if (err) {
                        console.log(err);
                        reject(err)
                    } else {
